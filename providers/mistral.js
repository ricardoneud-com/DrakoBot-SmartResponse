...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const instance = createMistral({ apiKey: smartResponse.config.Providers.Mistral.APIKey });
        ...
        const model = instance(smartResponse.config.Providers.Mistral.Model);
        const { text: content } = await generateText({
            model,
            messages,
            temperature: smartResponse.config.Providers.Mistral.Temperature,
            maxTokens: smartResponse.config.Providers.Mistral.MaxTokens
        });
