...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const apiKey = smartResponse?.config?.Providers?.Mistral?.APIKey;
        const modelName = smartResponse?.config?.Providers?.Mistral?.Model;
        const temperature = smartResponse?.config?.Providers?.Mistral?.Temperature;
        const maxTokens = smartResponse?.config?.Providers?.Mistral?.MaxTokens;

        if (!apiKey || !modelName) {
            console.error('[ERROR] Missing Mistral API key or model name in configuration');
        ...
        try {
            ...
            console.error('[ERROR] Mistral API request failed:', apiError);
