...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const apiKey = smartResponse?.config?.Providers?.Groq?.APIKey;
        const modelName = smartResponse?.config?.Providers?.Groq?.Model;
        const temperature = smartResponse?.config?.Providers?.Groq?.Temperature;
        const maxTokens = smartResponse?.config?.Providers?.Groq?.MaxTokens;

        if (!apiKey || !modelName) {
            console.error('[ERROR] Missing Groq API key or model name in configuration');
            return { type: 'TEXT', content: 'Configuration error: Missing API credentials or model.', hasNextSteps: false };
        }

        const instance = createGroq({ apiKey });
        ...
        try {
            ...
            console.error('[ERROR] Groq API request failed:', apiError);
        ...
        console.error('[FATAL] Unexpected error in Groq generateResponse function:', error);
