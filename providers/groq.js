...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const instance = createGroq({ apiKey: smartResponse.config.Providers.Groq.APIKey });
        ...
        const model = instance(smartResponse.config.Providers.Groq.Model);
        const { text: content } = await generateText({
            model,
            messages,
            temperature: smartResponse.config.Providers.Groq.Temperature,
            maxTokens: smartResponse.config.Providers.Groq.MaxTokens,
        });
