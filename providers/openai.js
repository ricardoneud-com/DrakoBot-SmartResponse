...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const apiKey = smartResponse?.config?.Providers?.OpenAI?.APIKey;
        const modelName = smartResponse?.config?.Providers?.OpenAI?.Model;
        const temperature = smartResponse?.config?.Providers?.OpenAI?.Temperature;
        const maxTokens = smartResponse?.config?.Providers?.OpenAI?.MaxTokens;

        if (!apiKey || !modelName) {
            console.error('[ERROR] Missing OpenAI API key or model name in configuration');
        ...
        try {
            ...
            console.error('[ERROR] OpenAI API request failed:', apiError);
