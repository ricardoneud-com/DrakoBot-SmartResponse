...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const instance = createOpenAI({ apiKey: smartResponse.config.Providers.OpenAI.APIKey });
        ...
        const model = instance(smartResponse.config.Providers.OpenAI.Model);
        const { text: content } = await generateText({
            model,
            messages,
            temperature: smartResponse.config.Providers.OpenAI.Temperature,
            maxTokens: smartResponse.config.Providers.OpenAI.MaxTokens
        });
