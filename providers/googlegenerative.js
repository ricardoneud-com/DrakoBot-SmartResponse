...
async function generateResponse(smartResponse, message, phrase) {
    try {
        ...
        const apiKey = smartResponse?.config?.Providers?.GoogleGenerative?.APIKey;
        const modelName = smartResponse?.config?.Providers?.GoogleGenerative?.Model;
        const temperature = smartResponse?.config?.Providers?.GoogleGenerative?.Temperature;
        const maxTokens = smartResponse?.config?.Providers?.GoogleGenerative?.MaxTokens;

        if (!apiKey || !modelName) {
            console.error('[ERROR] Missing Google Generative API key or model name in configuration');
        ...
        try {
            ...
            console.error('[ERROR] Google Generative API request failed:', apiError);
        ...
        console.error('[FATAL] Unexpected error in Google Generative generateResponse function:', error);
